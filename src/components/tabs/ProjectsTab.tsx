...
const emptyProjectForm = { name: '', description: '', status: 'in_progress' as Project['status'] }
const emptyTaskForm = { name: '', status: 'planned' as ProjectTask['status'] }

export default function ProjectsTab({ projects, projectTasks, onRefresh }: Props) {
  ...
  const closeProjectModal = () => { setProjectModal(null); setSelectedProject(null); setProjectForm(emptyProjectForm) }
  const closeTaskModal = () => { setTaskModal(null); setSelectedTask(null); setSelectedProjectId(null); setTaskForm(emptyTaskForm) }

  const saveProject = async () => {
    setSaving(true)
    const payload = { name: projectForm.name, description: projectForm.description || null, status: projectForm.status }
    if (projectModal === 'add') {
      await supabase.from('projects').insert({ ...payload, order_index: projects.length })
  ...
  const saveTask = async () => {
    ...
    if (taskModal === 'add') {
      await supabase.from('project_tasks').insert({ ...payload, project_id: selectedProjectId, order_index: getTasks(selectedProjectId).length })
    ...
    <>
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => openProjectModal('add')} className="text-indigo-500 hover:text-indigo-700 transition-colors">
            <Plus size={18} />
          </button>
        </div>
        {projects.map((p) => (
