...
export default function HeroCard({
  ...
  const gapColor =
    gapPct === null ? 'bg-white/30' : gapPct >= 70 ? 'bg-emerald-300' : gapPct >= 40 ? 'bg-amber-300' : 'bg-rose-300';
  ...
  return (
    <div className="lg:col-span-2 bg-gradient-to-br from-indigo-500 to-violet-500 rounded-2xl p-5 text-white relative overflow-hidden">
      ...
      <p className="text-xs opacity-70 mb-4">{settings.big_goal_sub ?? '시니어 → 리드 → 아키텍트'}</p>
            ...
            {gapPct !== null && (
              <div className="flex items-center gap-3">
                <div className="flex-1 h-1.5 bg-white/20 rounded-full overflow-hidden">
                  <div className={`h-full rounded-full transition-all ${gapColor}`} style={{ width: `${gapPct}%` }} />
                </div>
