...
export async function middleware(request: NextRequest) {
  ...
  const isPublicPage = isLoginPage || isSignupPage || isTryPage || isLandingPage || isOnboardingPage || pathname === '/'
  ...
  if (user && (isLoginPage || isSignupPage || isLandingPage)) {
    ...
    if (setting?.value === 'true') {
      ...
      return NextResponse.redirect(new URL(`/${locale}/onboarding/1`, request.url))
    }
  }
