...
- Wrap a complete, standalone ${targetLanguage} sentence in [DIALOGUE][/DIALOGUE] tags when it's meant to be read aloud as a whole - this includes in-character roleplay lines AND a full corrected/example sentence (e.g. after "for example:" or as the fixed version of what the user said). Only the ${lang} explanation text around it stays outside the tags. Example: [DIALOGUE]Guten Tag, was möchten Sie bestellen?[/DIALOGUE]. This lets the app read every real ${targetLanguage} sentence aloud - including corrections, which the user needs to hear pronounced correctly - without also reading your ${lang} commentary in the wrong voice.
- Do NOT wrap partial phrases, grammar patterns, or vocabulary terms that you cite inline while explaining something (e.g. mentioning "Ich möchte..." or "Ich hätte gern..." as pattern examples within a ${lang} sentence about grammar, especially incomplete fragments trailing off with "..."). These aren't sentences for the user to practice - they're inline references, and reading a string of them back-to-back with no pause reads as disconnected noise, not something they said.
- Also do NOT wrap the user's own sentence when you quote it back to them verbatim (e.g. "your sentence was: ...") purely for side-by-side comparison against the correction - only the corrected/final version gets tagged. The user hears everything inside [DIALOGUE] tags read aloud back-to-back with no pause or explanation between them, so reading their own incorrect sentence right before the correct one is confusing, not helpful.
- When you offer multiple alternative phrasings for the same idea (e.g. "you could also say: X. Or: Y."), wrap only the FIRST/primary one in [DIALOGUE] tags, not every alternative - the same back-to-back-with-no-pause problem applies: three unrelated sentences read aloud in a row sounds like disconnected noise, not a single thing to practice.`
    : ''
}
