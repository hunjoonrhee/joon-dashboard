...
export default function TryPage() {
  ...
  const inputCls =
    'w-full border border-gray-200 rounded-lg px-3.5 py-2.5 text-sm text-gray-800 placeholder-gray-400 outline-none focus:border-indigo-400 bg-white transition-colors'
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm mb-4">
              ...
              <div className="flex flex-col gap-2">
                {stages.map((stage, i) => (
                  <div
                    key={i}
                    className={`flex items-start gap-3 p-3 rounded-xl border ${i === stages.length - 1 ? 'bg-indigo-50 border-indigo-200' : 'bg-gray-50 border-gray-100'}`}
                  >
                    <div
                      className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5 ${i === stages.length - 1 ? 'bg-indigo-500 text-white' : 'bg-gray-200 text-gray-500'}`}
                    >
                      {stage.level}
                    </div>
                    <div className="min-w-0">
                      <p
                        className={`text-sm font-semibold ${i === stages.length - 1 ? 'text-indigo-700' : 'text-gray-800'}`}
                      >
                      ...
                      <div className="flex gap-1 flex-wrap mt-1.5">
                        {stage.skills
                          ...
                          .map((tag, j) => (
                            <span
                              key={j}
                              className="text-xs px-1.5 py-0.5 rounded-full bg-indigo-100 text-indigo-600"
                            >
                              {tag}
                            </span>
                          ))}
