    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="max-w-2xl mx-auto text-center px-6 pt-20 pb-16">
        ...
        <div className="flex gap-3 justify-center flex-wrap">
          <button onClick={() => router.push(`/${locale}/try`)}
        ...
        <div className="bg-gray-900 border border-white/7 rounded-2xl p-5">
          <div className="grid grid-cols-3 gap-3 mb-3">
            ...
            <div className="bg-gray-800 rounded-xl p-3 border border-white/5">
              ...
              <p className="text-sm font-semibold text-indigo-300 mt-1">NgRx 공부해</p>
              <p className="text-xs text-gray-500 mt-1">{t('previewAISub')}</p>
            </div>
          </div>
          <div className="bg-gray-800 rounded-xl p-3 border border-white/5">
            <p className="text-xs text-gray-500 mb-2">Lead Architekt {t('previewRoadmap')}</p>
            <div className="flex gap-1.5">
              {[1,2,3,4,5].map(i => (
                <div key={i} className={`flex-1 h-1.5 rounded-full ${i <= 2 ? 'bg-indigo-500' : 'bg-gray-700'}`} />
              ))}
            </div>
            <div className="flex justify-between mt-1.5">
              <span className="text-xs text-indigo-400">{t('previewStage2')}</span>
              <span className="text-xs text-gray-500">{t('previewStage5')}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-6 mb-20">
        <div className="text-center mb-10">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-green-500/10 border border-green-500/20 text-green-400 text-xs font-semibold mb-3">핵심 기능</span>
          <h2 className="text-2xl font-bold tracking-tight">{t('featTitle')}</h2>
        </div>
        <div className="grid grid-cols-3 gap-4">
          {[
            { icon: '🗺️', title: t('feat1Title'), desc: t('feat1Desc'), bg: 'bg-indigo-500/10' },
            { icon: '📊', title: t('feat2Title'), desc: t('feat2Desc'), bg: 'bg-green-500/10' },
            { icon: '🤖', title: t('feat3Title'), desc: t('feat3Desc'), bg: 'bg-amber-500/10' },
          ].map((f, i) => (
            <div key={i} className="bg-gray-900 border border-white/7 rounded-2xl p-5">
